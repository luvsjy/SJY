...
class Pix2PixGLGModel(BaseModel):
    ...
    #
    #     # 获取 .npy 文件路径
    #     image_path = input['A' if AtoB else 'B']
    #     mask_path = input['B' if AtoB else 'A']
    #
    #     # 加载 .npy 文件
    #     self.images = torch.from_numpy(np.load(image_path)).to(self.device)
    #     self.masks = torch.from_numpy(np.load(mask_path)).to(self.device)
    #     # 记录路径（如果需要）
    #     self.image_paths = input['A_paths' if AtoB else 'B_paths']
