...
class Pix2PixGSGModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.优化器初始化
    ...
    def forward(self, ablation=None):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        target_size = self.masks.shape[-2:]  # 获取 masks 的目标尺寸

        # 如果图像尺寸与目标尺寸不一致，进行插值
        if self.images.shape[-2:] != target_size:
            self.images = torch.nn.functional.interpolate(self.images, size=target_size, mode='bilinear',
                                                          align_corners=False)

        # 使用加载的掩码进行图像融合处理
        self.masked_images1 = self.images * (1 - self.masks) + self.masks * 0
        self.output_images1 = self.netG1(self.masked_images1)
        self.merged_images1 = self.images * (1 - self.masks) + self.output_images1 * self.masks
        # ablation='g3'
        if ablation == 'g1':
            self.output_images2 = self.output_images1
            self.merged_images2 = self.merged_images1
            self.output_images3 = self.output_images1
            self.merged_images3 = self.merged_images1
        elif ablation == 'g1_g2':
            self.output_images2 = self.netG2(self.merged_images1)
            self.merged_images2 = self.images * (1 - self.masks) + self.output_images2 * self.masks
            self.output_images3 = self.output_images2
            self.merged_images3 = self.merged_images2
        elif ablation == 'g1_g3':
            self.output_images2 = self.output_images1
            self.merged_images2 = self.merged_images1
            self.output_images3 = self.netG3(self.merged_images1)
            self.merged_images3 = self.images * (1 - self.masks) + self.output_images3 * self.masks
        elif ablation == 'g2':
            self.output_images2 = self.netG2(self.masked_images1)
            self.merged_images2 = self.images * (1 - self.masks) + self.output_images2 * self.masks
            self.output_images3 = self.output_images2
            self.merged_images3 = self.merged_images2
        elif ablation == 'g2_g3':
            self.output_images2 = self.netG2(self.masked_images1)
            self.merged_images2 = self.images * (1 - self.masks) + self.output_images2 * self.masks
            self.output_images3 = self.netG3(self.merged_images2)
            self.merged_images3 = self.images * (1 - self.masks) + self.output_images3 * self.masks
        elif ablation == 'g3':
            self.output_images2 = self.output_images1
            self.merged_images2 = self.merged_images1
            self.output_images3 = self.netG3(self.masked_images1)
            self.merged_images3 = self.images * (1 - self.masks) + self.output_images3 * self.masks
        else:
            self.output_images2 = self.netG2(self.merged_images1)
            self.merged_images2 = self.images * (1 - self.masks) + self.output_images2 * self.masks
    ...
    def backward_G(self, ablation=None):
    ...
    def optimize_parameters(self, ablation=None):
        self.forward(ablation)  # compute fake images: G(A)
        ...
        self.backward_G(ablation)  # calculate gradients for G
        self.optimizer_G1.step()  # update G's weights
        self.optimizer_G2.step()  # update G's weights
        self.optimizer_G3.step()  # update G's weights