...
class SeismicDataset:
    def __init__(self, sgy_file, output_dir, block_size=(256, 256), num_npy_files=None):
        ...
        self.npy_dir = os.path.join(output_dir, "Test")
    ...
    def save_blocks_as_npy_and_images(self, blocks):
        ...
        mask_counts = {mask_ratio: 0 for mask_ratio in mask_ratios}  # 每个掩码比例的计数器

        for mask_ratio in mask_ratios:
            for i, block in enumerate(blocks):
                if mask_counts[mask_ratio] >= 1000:
                    break  # 达到每个掩码比例的文件数量限制时停止
                if npy_count >= 5000:
                    return  # 达到总文件数量限制时停止

                npy_path = os.path.join(self.npy_dir, f"block_{npy_count}.npy")
                ...
                img_path = os.path.join(self.img_dir, f"block_{npy_count}.png")
                img.save(img_path)

                mask = self.generate_random_mask(block, mask_ratio)

                mask_npy_path = os.path.join(self.mask_npy_dir, f"block_{npy_count}_mask.npy")
                np.save(mask_npy_path, mask)

                masked_img = Image.fromarray(mask.astype(np.uint8) * 255)
                masked_img_path = os.path.join(self.mask_img_dir, f"block_{npy_count}_mask.png")
                ...
                test_msk_path = os.path.join(mask_class_dir, f"block_{npy_count}_mask.npy")
                ...
                test_msk_img_path = os.path.join(mask_img_class_dir, f"block_{npy_count}_mask.png")
                masked_img.save(test_msk_img_path)

                npy_count += 1
                mask_counts[mask_ratio] += 1  # 更新掩码类别的计数器
...
# 使用示例
sgy_file = r"D:\SoftWare\ChengxuXiangmu\dataset\sesimic_data\Model94_shots.segy"
output_dir = r"D:\SoftWare\ChengxuXiangmu\LG_Data\xianchang1_Test"
num_npy_files = 1000  # 假设我们只想生成100个npy文件
